...
const PlotsPage: React.FC = () => {
  ...
  const fetchPlotProjects = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getProjects('plot');
      
      if (response.success && response.data && response.data.projects) {
        setProjects(response.data.projects);
      } else {
        setProjects([]);
        setError(response.message || 'Failed to fetch plot projects');
      }
    } catch (err) {
      setProjects([]);
      setError('Error loading projects');
      console.error('Error fetching plot projects:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading plot projects...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      {!projects || projects.length === 0 ? (
        <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-12 text-center">
          <p className="text-gray-500 text-lg">No plot projects found</p>
          <p className="text-gray-400 text-sm mt-2">Create plot projects from the Projects page</p>
        </div>
      ) : (
